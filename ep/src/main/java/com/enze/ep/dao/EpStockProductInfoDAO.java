...
public interface EpStockProductInfoDAO {
    ...
    @Select({"select ", SELECT_FIELDS, " from ", TABLE_NAME, " where iproductid = #{iproductid} and icounterid in (select icounterid from tb_counter where sectionid =#{sectionid}) and vcproductunit=#{iunitid} and  NUMSTOCKS>0 and isnull(FlagStop,'N')='N' order by dtusefulllife "})
    List<TbStockProductInfo> selectStockProductInfoListByProductIDAndSectionidAndUnitID(int iproductid,int  sectionid,int iunitid);

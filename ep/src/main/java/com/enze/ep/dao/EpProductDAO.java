...
public interface EpProductDAO {
    ...
    @Select({"select ", SELECT_FIELDS, " from tb_Productinfo a left join tb_StockProductInfo b ", "on a.IPRODUCTID = b.IPRODUCTID"," where  b.ICOUNTERID in (#{counterids}) and b.NUMSTOCKS>0 and (a.VCUNIVERSALNAME like '%#{productName}%' or a.VCPRODUCTNAME like '%#{productName}%' or a.VCEASYCODE like '%#{productName}%' or a.VCPRODUCTCODE like '%#{productName}%' or a.VCSTANDARD like '%#{productName}%')"})

...
public class QueryJob {
	...
	// 10秒钟之后开始，每5秒一次 检查支付订单在平台上的处理情况
	...
	public void queryOrderPayState() {
		...
		List<EpOrder> list = epOrderServiceImpl.findOrderListByOrderTypeAndUsestatusAndMinutesAOB(ordertype, usestatus,
				minutes, flagsendstore);
	...
	public void queryOrderRefundWeixinState() {
		...
		int usestatus = EpOrderUsestatus.initial;
		int minutes = -1000;
		int weixinrefundappflag = 1;
		// int flagsendstore=0;
		// 查询未结账的销售单据 时间 类型等因数查询
		List<EpOrder> list = epOrderServiceImpl.findOrderByOrderTypeAndUsestatusAndMinutesAndWeixinRefundAppFlag(
				ordertype, usestatus, minutes, weixinrefundappflag);
		for (EpOrder order : list) {
			doRefundQuery(order);
		}
		log.info("Start queryOrderRefundWeixinState 微信退款状态查询");
	...
	//查询微信退款申请订单状态
	private void doRefundQuery(EpOrder order) {
		new Runnable() {
			@Override
			public void run() {
